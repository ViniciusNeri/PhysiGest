...
class ExerciseBloc extends Bloc<ExerciseEvent, ExerciseState> {
  ...
  List<Exercise> _allExercises = [
    const Exercise(
      ...
      description: 'Movimentos suaves de rotação e inclinação para aliviar tensão no pescoço. Ideal para quem trabalha horas no computador.',
    ...
    const Exercise(
      ...
      description: 'Uso de elástico ou mola para rotação externa. Auxilia muito na estabilidade e evita pinçamentos no ombro.',
    ...
    const Exercise(
      ...
      description: 'Deitado de barriga para cima, elevar o quadril ativando os glúteos e abdômen. Ajuda a estabilizar a lombar.',
    ...
    const Exercise(
      ...
      description: 'Deitado, elevar uma perna estendida puxando a ponta do pé com toalha ou faixa.',
      videoUrl: 'https://www.youtube.com/watch?v=placeholder4',
      thumbnailUrl: 'https://images.unsplash.com/photo-1544367567-0f2fcb009e0b?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80',
    ...
    const Exercise(
      ...
      description: 'Estimulação de oxigenação e expansão da caixa torácica inferior com as mãos sob as costelas.',
      videoUrl: 'https://www.youtube.com/watch?v=placeholder5',
      thumbnailUrl: 'https://images.unsplash.com/photo-1499209974431-9dddcece7f88?ixlib=rb-4.0.3&auto=format&fit=crop&w=800&q=80',
  ...
  void _onAddExercise(AddExercise event, Emitter<ExerciseState> emit) {
    if (state is ExerciseLoaded) {
      ...
      emit(ExerciseLoaded(
        exercises: _getFilteredExercises(currentState.searchQuery, currentState.selectedCategory),
  ...
  void _onSearchExercises(SearchExercises event, Emitter<ExerciseState> emit) {
    if (state is ExerciseLoaded) {
      final currentState = state as ExerciseLoaded;
      emit(ExerciseLoaded(
        exercises: _getFilteredExercises(event.query, currentState.selectedCategory),
  ...
  void _onFilterByCategory(FilterByCategory event, Emitter<ExerciseState> emit) {
    if (state is ExerciseLoaded) {
      final currentState = state as ExerciseLoaded;
      emit(ExerciseLoaded(
        exercises: _getFilteredExercises(currentState.searchQuery, event.category),
        searchQuery: currentState.searchQuery,
  ...
  List<Exercise> _getFilteredExercises(String? query, String? category) {
    return _allExercises.where((e) {
      final matchesQuery = query == null || query.isEmpty || e.title.toLowerCase().contains(query.toLowerCase());
      final matchesCategory = category == null || category == 'Todos' || e.category == category;
